        ...
        for i in range(0, 4):
            ...
            for t2 in rotatedsingles:
                ...
                for j in range(0, 4):
                    ...
                    spairs['1NGO'].add(frozenset((_comp(t2.ends[j]), _comp(t1.ends[j]))))
                    go1 = False
                    if t1['input'][i] and t1['input'][j]:
                        go1 = True
                        spairs['1GO'].add(frozenset((t2.ends[j], t1.ends[j])))
                        spairs['1GO'].add(frozenset((_comp(t2.ends[j]), _comp(t1.ends[j]))))
                        ...
                        for t12 in singles:
                            ...
                            for t22 in rotatedsingles:
                                ...
                                for m in range(0, 4):
                                    ...
                                    spairs['2NGO'].add(
                                        frozenset((_comp(t2.ends[j]), _comp(t1.ends[j]))))
